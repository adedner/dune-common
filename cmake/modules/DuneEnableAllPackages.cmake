...
#    .. cmake_brief::
#
#       Previously, the DUNE build system relied on the user to choose and add the compile and link
#       flags necessary to build an executable. While this offers full control to the user, it
#       is an error-prone procedure.
#
#       Alternatively, users may use this function to simply add the compile flags for all
#       found external modules to all executables in a DUNE module. Likewise, all found libraries
#       are linked to all targets.
#
...
#       :multi:
#
#       If your module contains libraries as well as programs and if the programs should
#       automatically link to those libraries, you *MUST* list these libraries in
#       :code:`MODULE_LIBRARIES`. Those libraries will be automatically created by
#       :ref:`dune_enable_all_packages` (which internally calls :ref:`dune_add_library`) and placed
#       in the lib/ directory. The order of the libraries matters: if one library depends on
#       another one, it must be listed after its dependency. This special handling of the libraries
#       is due to the way CMake handle linking (in particular CMP022 and CMP038). You can later add
#       source files to the library anywhere in the source tree by calling
#       :ref:`target_sources`.
#
#    .. cmake_param:: VERBOSE
#       :option:
...
#       appended to the global collection of flags instead of being prepended. Only use it, if you
#       know what you are doing.
#
#    Adds all flags and all libraries to all executables that are subsequently added in the
#    directory from where this function is called and from all its subdirectories (recursively).
#    If used, this function *MUST* be called in the top level CMakeLists.txt BEFORE adding any
#    subdirectories! You can optionally add additional include dirs and compile definitions that
#    will also be applied to all targets in the module.
#
#    .. note::
#       If you want to use :code:`dune_enable_all_packages` with an older version of CMake and your
#       DUNE module creates its own library, you have to manually create the library in the
#       top-level CMakeLists.txt file using :ref:`dune_add_library` (with all sources listed within
#       that call), use :ref:`dune_target_enable_all_packages` to add all packages to the library
#       and finally list that library under :code:`LIBRARIES` in the call to
#       :ref:`dune_register_package_flags`. See dune-pdelab for an example of
#       how to do this correctly.
#
#    While :ref:`dune_enable_all_packages` defines the user interface for this feature, developers
#    might also be interested in the following related functions:
#
...
#
#    Adds all currently registered package flags (see :ref:`dune_register_package_flags`) to the
#    given targets. This function is mainly intended to help write DUNE modules that want to use
#    :ref:`dune_enable_all_packages` and define their own libraries, but need to be compatible with
#    CMake < 3.1
#
#
...
#    include paths and link flags of all found packages must be registered with this function. This
#    function is only necessary for people that want to write their own :code:`FindFooBar` CMake
#    modules to link against additional libraries which are not supported by the DUNE core modules.
#    Call this function at the end of every find module. If you are using an external FindFoo
...
function(dune_enable_all_packages)
  ...
  cmake_parse_arguments(DUNE_ENABLE_ALL_PACKAGES
    "APPEND;VERBOSE" "" "${MULTIARGS}" ${ARGN})
  ...
  if(DUNE_ENABLE_ALL_PACKAGES_INCLUDE_DIRS)
    if(DUNE_ENABLE_ALL_PACKAGES_APPEND)
      set_property(GLOBAL APPEND PROPERTY ALL_PKG_INCS
        "${DUNE_ENABLE_ALL_PACKAGES_INCLUDE_DIRS}")
    else(DUNE_ENABLE_ALL_PACKAGES_APPEND)
      get_property(all_incs GLOBAL PROPERTY ALL_PKG_INCS)
      set_property(GLOBAL PROPERTY ALL_PKG_INCS
        "${DUNE_ENABLE_ALL_PACKAGES_INCLUDE_DIRS}" "${all_incs}")
  ...
  if(DUNE_ENABLE_ALL_PACKAGES_COMPILE_DEFINITIONS)
    if(DUNE_ENABLE_ALL_PACKAGES_APPEND)
      set_property(GLOBAL APPEND PROPERTY ALL_PKG_DEFS
        "${DUNE_ENABLE_ALL_PACKAGES_COMPILE_DEFINITIONS}")
    else(DUNE_ENABLE_ALL_PACKAGES_APPEND)
      get_property(all_defs GLOBAL PROPERTY ALL_PKG_DEFS)
      set_property(GLOBAL PROPERTY ALL_PKG_DEFS
        "${DUNE_ENABLE_ALL_PACKAGES_COMPILE_DEFINITIONS}" "${all_defs}")
    endif(DUNE_ENABLE_ALL_PACKAGES_APPEND)
  endif(DUNE_ENABLE_ALL_PACKAGES_COMPILE_DEFINITIONS)

  # add compile definitions to all targets in module
  get_property(all_defs GLOBAL PROPERTY ALL_PKG_DEFS)
  # We have to do this in a loop because add_definitions() is kind of broken: even though it is
  # supposed to be *the* function for adding compile definitions, it does not prepend "-D" (as
  # opposed to target_compile_definitions(), which does). Well, whatever...
  ...
  if(DUNE_ENABLE_ALL_PACKAGES_COMPILE_OPTIONS)
    if(DUNE_ENABLE_ALL_PACKAGES_APPEND)
      set_property(GLOBAL APPEND PROPERTY ALL_PKG_OPTS
        "${DUNE_ENABLE_ALL_PACKAGES_COMPILE_OPTIONS}")
    else(DUNE_ENABLE_ALL_PACKAGES_APPEND)
      get_property(all_opts GLOBAL PROPERTY ALL_PKG_OPTS)
      set_property(GLOBAL PROPERTY ALL_PKG_OPTS
        "${DUNE_ENABLE_ALL_PACKAGES_COMPILE_OPTIONS}" "${all_opts}")
    endif(DUNE_ENABLE_ALL_PACKAGES_APPEND)
  ...
  # of default libraries to link to. That works because calling link_libraries does not affect
  # targets which already exist.
  # Moroever, CMake generates a warning when creating a library without any source files, and the
  # linker does the same if we add an empty dummy file. We work around that problem by
  # autogenerating a library-specific stub source file with two functions ${lib_name}_version() and
  # ${lib_name}_version_string() and add that as an initial source file.
  # After creating the library with dune_add_library(), we add it to all future targets with a call
  # to link_libraries(). The user can then add the real source files by calling
  # target_sources() throughout the module.
...
function(dune_library_add_sources lib)
  ...
  if (NOT (DEFINED DUNE_ENABLE_ALL_PACKAGES_MODULE_LIBRARIES))
    message(FATAL_ERROR "You must call dune_enable_all_packages with the MODULE_LIBRARIES option "
      "before calling dune_library_add_sources")
  endif()

  if (NOT lib IN_LIST DUNE_ENABLE_ALL_PACKAGES_MODULE_LIBRARIES)
    message(FATAL_ERROR "Attempt to add sources to library ${lib}, which has not been defined "
      "in dune_enable_all_packages. List of libraries defined in dune_enable_all_packages: "
      "${DUNE_ENABLE_ALL_PACKAGES_MODULE_LIBRARIES}")
  endif()
