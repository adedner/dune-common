...
template <class X, class F = typename X::field_type>
...
template <class N, class X, class R = typename X::real_type>
...
template <class N, class X, class F = typename X::field_type, class R = typename X::real_type>
...
template <class I, class X, class F = typename X::field_type>
...
struct InducedNorm
{
  ...
  auto operator()(const X& x) const
    -> decltype(square_root(std::declval<I>()(x,x)))
  {
    I inner;
    return square_root(inner(x,x));
  }

private:
  template <class F>
  static auto square_root(const F& f)
  {
    using std::sqrt;
    return sqrt(f);
  }
...
template <class I, class X, class F = typename X::field_type, class R = typename X::real_type>
concept HilbertSpace = BanachSpace<InducedNorm<I>,X,F,R> && InnerProductSpace<I,X,F>;
