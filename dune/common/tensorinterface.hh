...
#ifndef DUNE_COMMON_TENSORINTERFACE_HH
#define DUNE_COMMON_TENSORINTERFACE_HH
...
protected:
  // A tag to allow calls to "protected" member functions of derived type
...
public:
  ...
  {
    return asDerived().access(accessor_type{}, indices...);
  }
  ...
  constexpr reference operator[] (const std::array<Index,extents_type::rank()>& indices)
  {
    return std::apply([&](auto... ii) -> reference {
      return asDerived().access(accessor_type{}, index_type(ii)...); }, indices);
  }
  ...
  {
    return ScalarType(asDerived().access(accessor_type{}));
  }
...
private:
  ...
  [[nodiscard]] constexpr bool indexInIndexSpace (Indices... indices) const noexcept
  {
    return unpackIntegerSequence([&](auto... i) {
      return ( (0 <= indices && index_type(indices) < asDerived().extent(i)) && ... );
  ...
  template <class L = layout_type, class D, std::size_t i0, std::size_t i1, std::size_t... ii,
    std::enable_if_t<not std::is_same_v<L, Std::layout_right>, int> = 0>
  static constexpr auto access (D&& /*derived*/, index_type /*index*/, std::index_sequence<i0,i1,ii...>)
  {
  ...
  template <class D, std::size_t i0>
  static constexpr decltype(auto) access (D&& derived, index_type index, std::index_sequence<i0>)
  {
    return derived.access(accessor_type{}, index);
  }

  // specialization for rank==0
  template <class D>
  static constexpr value_type access (D&& /*derived*/, index_type /*index*/, std::index_sequence<>)
  {
...
#endif // DUNE_COMMON_TENSORINTERFACE_HH
